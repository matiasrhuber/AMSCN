...
class PacketArrival(SimEvent):
    ...
    def process(self):
        ...
        #######################################
        # DONE Task 2.2.3:
        # generate next packet for this user
        self.user.generate_packet_arrival()
        packet = self.user.generate_packet()
        # try to start service; if server is busy, add packet to buffer
        if not self.sim.sys_state.start_serving_packet(packet):
            self.sim.sys_state.add_packet_to_buffer(packet)
        #######################################

        self.sim.sys_state.count_arrival()
